...
function moveItemToEnd(arr, index) {
  ...
  let endLetter = arr.splice(index, 1);
  return [...arr, ...endLetter];
}
...
// Exercise 16
function updateUserAge(userAccount) {
  /*
  ...
  */
  userAccount.personalDetails.age += 1;
}
...
function checkInfinitive(word) {
  ...
  return /\b.*(re|ir|er)\b/g.test(word);
}
...
function collectPlurals(strings) {
  ...
  return strings.filter((string) => /\b.*s$/g.test(string));
}

runTest("collectPlurals() can collect all the strings ending in an s", function () {
  check(collectPlurals(["dogs", "cat", "apples", "kittens", "kiwi"])).isEqualTo([
    "dogs",
    "apples",
    "kittens",
  ]);
...
// Exercise 19
function makeAllAdmins(usersArray) {
  /*
  ...
  */
  return usersArray.map((user) => ({ ...user, admin: true }));
}

runTest("makeAllAdmins() updates the admin property for each user", function () {
  check(
    makeAllAdmins([

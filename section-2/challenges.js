...
runTest("roundDown() returns the result of rounding down to the nearest integer", function () {
...
runTest("isInThe20thCentury() checks if a number is within 1901 to 2000 (inclusive)", function () {
...
runTest("isAbsolutePath() checks if a file path is absolute or relative", function () {
  ...
  check(isAbsolutePath("/Users/mitch/northcoders/remote_course/remote_precourse_1")).isEqualTo(
    true
  );
...
function checkBatteryLevel(percentage) {
  if (percentage <= 5) {
    return `Warning - battery level low: ${percentage}%, please charge your device`;
  } else if (percentage > 5 && percentage <= 99) {
